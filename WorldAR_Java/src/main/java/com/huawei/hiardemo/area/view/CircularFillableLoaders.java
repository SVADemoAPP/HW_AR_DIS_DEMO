...
public class CircularFillableLoaders extends ImageView {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (waveShader != null) {
            ...
            String temp = String.valueOf(mProgress) + "%";
            Resources resources = getContext().getResources();
            float scale = resources.getDisplayMetrics().density;
            mBPaint.setTextSize(30*scale);
            Rect rect = new Rect();
            mBPaint.getTextBounds(temp, 0, temp.length(), rect);
            int w = rect.width() / 2;
            int h = rect.height() / 2;
            canvas.drawText(temp, (width / 2 - w), (height / 2 + h), mBPaint);
        } else {
